...
const ThumbOg = forwardRef(
  ({ activeState, backgroundImage = null, dark = true }, ref) => {
    const stageRef = useRef(null);
    const layout = useMemo(() => {
      if (!activeState) return null;
      const PHONE_WIDTH = THUMBNAIL_WIDTH * 0.3525;
      const PHONE_HEIGHT = THUMBNAIL_WIDTH * 0.8;
      const qrScale = Math.max(0.1, Math.min(1, activeState.qr.scale || 0.5));
      const minPhoneDimension = Math.min(PHONE_WIDTH, PHONE_HEIGHT);
      const QR_SIZE = minPhoneDimension * qrScale; 
    ...
    const exportAsBlob = async (options = {}) => {
      ...
      try {
        ...
        console.log(
          `✅ Thumbnail exported as ${format}: ${Math.round(
            blob.size / 1024
          )}KB`
        );
        return blob;
            ...
            >
              ...
              >
                ...
                >
                  ...
                  />
                  <Path strokeWidth={1}
                    stroke={activeState?.qr.primaryColor}
                    fill={activeState?.qr.primaryColor}
                    data="M8 0H0V8H8V0Z"
                  />
                  <Path strokeWidth={1}

...
const [width, setWidth] = useState(panelSize.width);

const onResize = (event, {size}) => {
  setWidth(size.width);
  setPanelSize({width:width, height:panelSize.height});
};
  ...
  className="duration-100"
    ...
    handle={
      <div className="fixed z-150 right-0 top-0 h-full w-[10px] cursor-col-resize">
        <div className="fixed right-[2.5px] top-1/2 cursor-pointer z-200">
            <span className="absolute start-1/1 -translate-x-1/2 block w-5 h-7 flex items-center bg-white border border-gray-200 text-gray-400 rounded-md  hover:bg-gray-100 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-600 dark:hover:bg-neutral-900"
            onClick={togglePanel}
            >
              <svg
    ...
    resizeHandles={['e']}>
      ...
      <div
        id="bottom-panel"
        className={`hs-overlay min-sm:hidden fixed [--body-scroll:true] transition-all duration-375 transform w-screen z-100 bg-white shadow-[0_-1px_5px_0px_rgba(0,0,0,0.1)] rounded-t-2xl
